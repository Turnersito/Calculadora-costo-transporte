...
/**
 * Searches for location suggestions with Nominatim (countrycodes=cl & addressdetails=1).
 */
export async function searchLocations(query) {
  ...
  // 2. Fetch from Nominatim API with countrycodes=cl and addressdetails=1
...
/**
 * Calculates driving routes using OSRM API with alternatives support.
 * @returns {Promise<Array<{distanceKm: number, durationMins: number, polyline: Array<[number, number]>}>>}
 */
export async function calculateRouteOSRM(originCoords, destCoords) {
  ...
  const url = `${CONFIG.OSRM_ROUTE_URL}/${coordsString}?overview=full&geometries=geojson&alternatives=true`;

  console.log(`🚗 [RutaCost] Solicitando rutas (alternatives=true) a OSRM API: ${coordsString}`);

  const response = await fetch(url);
  if (!response.ok) {
    throw new Error('Servidor de rutas no disponible. Verifica tu conexión.');
  }

  const data = await response.json();
  if (!data.routes || data.routes.length === 0) {
    throw new Error('No se encontró una ruta terrestre conectada para estos dos puntos.');
  }

  // Map all returned alternative routes
